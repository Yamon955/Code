...
void pushup(int l,int r,int rt)
{
	...
	int mid = (l+r)>>1;
	if(a[mid] < a[mid+1]){
		if(tree[rt].lmax == mid-l+1)
			tree[rt].lmax += tree[rt<<1|1].lmax;
		if(tree[rt].rmax == r-mid)
			tree[rt].rmax += tree[rt<<1].rmax;
		tree[rt].len = max(tree[rt].len,tree[rt<<1].rmax+tree[rt<<1|1].lmax);
	}
}
void build(int l,int r,int rt)
{
	if(l == r){
		tree[rt].len = tree[rt].lmax = tree[rt].rmax = 1;
		return ;
	}
	int mid = (l+r)>>1;
	build(l,mid,rt<<1);
...
void update(int l,int r,int pos,int c,int rt)
{
	if(l == r){
		//a[pos] = c;
		return;
...
int query(int l,int r,int x,int y,int rt)
{
	...
	if(y <= mid) return query(l,mid,x,y,rt<<1);
		...
		int left = query(l,mid,x,y,rt<<1);
		int right = query(mid+1,r,x,y,rt<<1|1);
		int ans;
		ans = max(left,right);
		if(a[mid] < a[mid+1]){
			ans = max(ans,min(tree[rt<<1].rmax,mid-x+1)+min(tree[rt<<1|1].lmax,y-mid));
		}
		return ans;
	}
}
