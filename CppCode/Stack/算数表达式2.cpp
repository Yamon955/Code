...
int push(stack &s,double e)//操作数进栈 
{
	if(s.top-s.base>=s.stacksize){
...
int pop(stack &s,double &e)//操作数出栈 
{
	if(s.top==s.base) return error;
	e=*--s.top;
	return ok;
}
double gettop(stack &s)//取操作数栈顶 
{
	if(s.base!=s.top)
		return *(s.top-1);
}
char Gettop(Stack &s)//取运算符栈顶 
{
	if(s.base!=s.top)
		return *(s.top-1);
}
...
double operate(double a1,char theta,double b1)//（a1 'theta' b1）运算函数 
{
	double c;
	switch(theta){
		case'+':
...
int evaluateExpression(char c)//中缀表达式求值函数 
{
	Stack s;
	stack t;
	Initstack(s);//运算符栈 
